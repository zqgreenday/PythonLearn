...
class Modbus_Rtu():
    def mb_port_open(self):
        ...
        self.mbTimeout = convert(entrydata10)  # ms
        try:
            mb_port = serial.Serial(port=self.mbComPort, baudrate=self.baudrate, bytesize=self.databit,
    ...
    def read_coils(self, addr, num):
        try:
            for widget in frame2.winfo_children():
                widget.destroy()
            if num <= 64:
                rc = self.master.execute(self.mbId, cst.READ_COILS, addr, num)
                ...
                for y in range(num//8+1):
                    for x in range(8):
                        if a < num:
                            ...
                            break
                root.after(1000, lambda: self.read_coils(convert(entrydata1), convert(entrydata2)))
            else:
                tkinter.messagebox.showwarning(title='Warning', message='读取数量设定超范围，必须在64个以内！！')
        except Exception as e:
            tkinter.messagebox.showerror(title='Error', message='Modbus Error: '+str(e))

    def write_single_register(self, addr, data):
        try:
            self.master.execute(self.mbId, cst.WRITE_SINGLE_REGISTER, addr, output_value=data)
            wsr = self.master.execute(self.mbId, cst.READ_HOLDING_REGISTERS, addr, 1)
            label = tkinter.Label(frame1, width=5, bd=2, relief="sunken", text=str(wsr[0]))
            label.grid(row=5, column=1)
        except Exception as e:
            tkinter.messagebox.showerror(title='Error', message='Modbus Error: ' + str(e))

    def read_holding_registers(self, addr, num):
        try:
            for widget in frame3.winfo_children():
                widget.destroy()
            if num <= 24:
                rhg = self.master.execute(self.mbId, cst.READ_HOLDING_REGISTERS, addr, num)
                ...
                for y in range((num//3)+1):
                    for x in range(3):
                        if a < num:
                            ...
                            break
                root.after(1000, lambda: self.read_holding_registers(convert(entrydata5), convert(entrydata6)))
            else:
                tkinter.messagebox.showwarning(title='Warning', message='读取数量设定超范围，必须在24个以内！！')
        except Exception as e:
            tkinter.messagebox.showerror(title='Error', message='Modbus Error: ' + str(e))

    def write_single_coil(self, addr, data):
        try:
            self.master.execute(self.mbId, cst.WRITE_SINGLE_COIL, addr, output_value=data)
...
if __name__ == '__main__':
    ...
    button4 = tkinter.Button(frame1, text='Write Single Coil', width=15,
                             command=lambda: modbusrtu.write_single_coil(convert(entrydata7), convert(entrydata8)))
    ...
    button1 = tkinter.Button(frame1, text='Read Coils', width=15,
                             command=lambda: modbusrtu.read_coils(convert(entrydata1), convert(entrydata2)))
    ...
    button2 = tkinter.Button(frame1, text='Write Single Reg', width=15,
                             command=lambda: modbusrtu.write_single_register(convert(entrydata3), convert(entrydata4)))
    ...
    button3 = tkinter.Button(frame1, text='Read Holding Reg', width=15,
                             command=lambda: modbusrtu.read_holding_registers(convert(entrydata5), convert(entrydata6)))
