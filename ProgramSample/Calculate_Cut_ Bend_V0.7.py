...
class Calculate():
    ...
    def set_cell_value(self, event):
        ...
        cn = int(str(column).replace('#', ''), 16)  # 计算列编号
        rn = int(str(row).replace('I', ''), 16)-self.k  # 计算行编号
        #print('cn,rn', cn, rn)
        entryedit = Text(self.mighty, width=10, height=1)  # 输入窗框设置数值
        entryedit.place(x=16+(cn-1)*80, y=6+rn*20)  # 数值输入组件放置位置
        ...
        def saveedit():
            ...
            self.remain_num[rn - 1] = self.number[rn - 1]
            entryedit.destroy()
            okb.destroy()
        okb = ttk.Button(self.mighty, text='OK', width=4, command=saveedit)  # 数值保存按钮
        okb.place(x=70+(cn-1)*80, y=2+rn*20)  # 数值保存按钮放置位置
    ...
    data0 = [width, length, number, state]
    filename1 = 'D:\data\data1.csv'
    data1 = [No_list, total_length, width_length, bend_length, specification, remain_length, remain_num_list]
    ...
    def calculated_value(self):
        if bool(self.width and self.length and self.number)==False:
            ...
            if (self.radSel == 1) and (self.n < 0):
                self.t3 = threading.Thread(target=self.auto_run())
                self.t3.start()
                self.cal_start['state'] = 'disabled'
            ...
            if self.remain_num[self.j] > 0:
                if self.i == 0:  # 第一组间隔条计算程序
                    ...
                    self.No_list.append(self.i//2 + 1)
                    self.treeview1.insert('', len(self.total_length) - 2,
                                          values=(self.No_list[len(self.total_length) - 2],
                    ...
                    self.treeview1.insert('', len(self.total_length) - 1,
                                          values=(self.No_list[len(self.total_length) - 1],
                    ...
                    if self.remain_num[self.j] == 0:
                        ...
                        for p in range(len(self.width)):  # 写入数据
                            self.treeview.insert('', p,
                                                 values=(self.width[p], self.length[p], self.number[p], self.state[p]))
    ...
    # 自动计算数据并发送
    def auto_run(self):
        if (self.cut_achieve == 1) and (self.n >= 0):  # 裁切设备就绪并且需生产窗框数量>0，将数据发送到PLC
    ...
    def clear_table(self):
        ...
        self.j_list.clear()
        self.k_list.clear()
        self.data0 = [self.width, self.length, self.number, self.state]
        self.data1 = [self.No_list,self.total_length, self.width_length, self.bend_length, self.specification,
                      self.remain_length, self.remain_num_list]
        self.data2 = [self.warehouse_no, self.warehouse_length]
        self.data3 = [self.i_list, self.j_list, self.k_list]
        self.i, self.j = 0, 0
    ...
    def read_table(self):
        try:
            ...
            for n in range(len(self.total_length)):  # 写入数据
                ...
                self.No_list[n], self.total_length[n], self.width_length[n], self.bend_length[n],
    ...
    def write_multiple_registers(self):
        if bool(self.total_length and self.width_length and self.bend_length )==False:
            ...
            try:
                data_cal = [self.No_list[self.i-2], self.total_length[self.i-2], self.width_length[self.i-2],
                            self.bend_length[self.i-2], self.specification[self.i-2], self.remain_length[self.i-2],
                            self.remain_num_list[self.i-2],
                            self.No_list[self.i-2], self.total_length[self.i - 1], self.width_length[self.i - 1],
    ...
    def com_data_update(self):
        ...
        self.datamonitor_win.after(500, self.com_data_update)
    ...
    def draw_sketch(self):
        ...
        path.goto(0, 155)
        path.color("GhostWhite")
        path.write(f"宽边：{self.width_length[frame_num-2]}mm", align='center', font=('Arial', 8, 'normal'))
        path.goto(self.width_length[frame_num-2]/-10, 150)
        path.down()
        path.setheading(180)
        path.color("GhostWhite")
    ...
    def sketch_map(self):
        if self.sketch_map_run == False:
            if len(self.i_list) <= 0:
                ...
                self.frame_num_list = [int(n/2) for n in set(self.i_list)]
    ...
    def creat_widgets(self):
        ...
        self.treeview.column("宽度", width=80, anchor='center')  # 表示列,不显示
        ...
        self.treeview.heading("宽度", text="宽度")  # 显示表头
        ...
        self.mighty3.grid(column=0, row=1, padx=4, pady=4, columnspan=2)
        self.scr = scrolledtext.ScrolledText(self.mighty3, width=146, height=14, wrap=WORD)
        ...
        self.manual_Rad = ttk.Radiobutton(self.mighty4, text='自动计算', variable=self.radVar, value=1,
                                          command=self.rad_call)
        self.manual_Rad.grid(row=1, column=1, sticky='W', padx=2, pady=2)
        self.manual_Rad = ttk.Radiobutton(self.mighty4, text='离线计算', variable=self.radVar, value=2,
                                          command=self.rad_call)
        self.manual_Rad.grid(row=2, column=0, sticky='W', padx=2, pady=2)
