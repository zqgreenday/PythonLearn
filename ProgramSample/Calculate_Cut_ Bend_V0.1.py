...
class Calculate():
    ...
    def __init__(self):
        self.root = Tk()
        self.root.title('间隔条裁切折弯计算软件-V0.1')
    ...
    i_list = []
    j_list = []
    filename0 = 'D:\data0.csv'
    data0 = [width, length, number, state]
    filename1 = 'D:\data1.csv'
    data1 = [total_length, width_length, bend_length, specification, remain_length, remain_num]
    filename2 = 'D:\data2.csv'
    data2 = [warehouse_no, warehouse_length]
    filename3 = 'D:\data3.csv'
    data3 = [i_list, j_list, k_list]
        ...
        if bool(self.width and self.length and self.number)==False:
            ...
            if self.number[self.j] > 0:
                if self.i == 0:  # 第一组间隔条计算程序
                    ...
                    self.remain_num.append(self.number[self.j])
                    ...
                    self.treeview1.insert('', len(self.total_length) - 2,
                                     values = (self.total_length[len(self.total_length) - 2], self.width_length[len(self.total_length) - 2],
                                              ...
                                              self.remain_length[len(self.total_length) - 2], self.remain_num[len(self.total_length) - 2]))
                    self.treeview1.insert('', len(self.total_length) - 1,
                                     values = (self.total_length[len(self.total_length) - 1], self.width_length[len(self.total_length) - 1],
                                              ...
                                              self.remain_length[len(self.total_length) - 1], self.remain_num[len(self.total_length) - 1]))
                    ...
                    for k in range(len(self.warehouse_length)):
                        ...
                        for k in range(len(self.warehouse_length)):
                            if self.warehouse_length[k] == 0:
                                self.warehouse_length[k] = self.remain_length[self.i]
                                break
                    self.bend_length.append((self.total_length[self.i] - self.width_length[self.i]) // 2)
                    self.remain_num.append(self.number[self.j])
                    self.treeview1.insert('', len(self.total_length) - 1,
                ...
                self.i += 2
                print(self.width, self.total_length,self.warehouse_length)
                print(self.data0, self.data1, self.data2)
            else:
    ...
    def clear_table(self):
        ...
        self.data1 = [self.total_length, self.width_length, self.bend_length, self.specification, self.remain_length, self.remain_num]
    ...
    def read_table(self):
        # 通过with语句读取，以列表类型读取
        with open(self.filename0, 'r') as fp:
            # 使用列表推导式，将读取到的数据装进列表
            data_list0 = [i for i in csv.reader(fp)]  # csv.reader 读取到的数据是list类型
        print(data_list0)
        self.width = [int(a) for a in data_list0[-4]]
        self.length = [int(a) for a in data_list0[-3]]
        self.number = [int(a) for a in data_list0[-2]]
        self.state = data_list0[-1]
        for _ in map(self.treeview.delete, self.treeview.get_children("")):
            pass
        for n in range(len(self.width)):  # 写入数据
            self.treeview.insert('', n, values=(self.width[n], self.length[n], self.number[n],
                                                 self.state[n]))

        # 通过with语句读取，以列表类型读取
        with open(self.filename1, 'r') as fp:
            # 使用列表推导式，将读取到的数据装进列表
        ...
        self.remain_num = [int(a) for a in data_list1[-1]]
        for _ in map(self.treeview1.delete, self.treeview1.get_children("")):
            pass
        for n in range(len(self.total_length)):  # 写入数据
            self.treeview1.insert('', n, values=(self.total_length[n], self.width_length[n], self.bend_length[n],
                                                 self.specification[n], self.remain_length[n], self.remain_num[n]))
        # 通过with语句读取，以列表类型读取
        with open(self.filename2, 'r') as fp:
            # 使用列表推导式，将读取到的数据装进列表
        ...
        with open(self.filename3, 'r') as fp:
            # 使用列表推导式，将读取到的数据装进列表
            data_list3 = [i for i in csv.reader(fp)]  # csv.reader 读取到的数据是list类型
        print(data_list3)
        self.i = int(data_list3[-3][-1])
        self.j = int(data_list3[-2][-1])
        #self.k = int(data_list3[-1][-1])

        self.data0 = [self.width, self.length, self.number, self.state]
        self.data1 = [self.total_length, self.width_length, self.bend_length, self.specification, self.remain_length,
                      self.remain_num]
        self.data2 = [self.warehouse_no, self.warehouse_length]
        self.data3 = [self.i_list, self.j_list, self.k_list]
    ...
    def creat_widgets(self):
        ...
        self.treeview1.grid(row=0, columnspan=3)
        self.ybar1.grid(row=0, column=3, sticky='ns')
        ...
        self.read_table = ttk.Button(self.mighty1, text='读取列表', width=15, command=self.read_table)
        self.read_table.grid(row=1, column=2, sticky='s')
